#!/usr/bin/env bash
# Build a Release AppsSwitch.app, Developer ID sign with Hardened Runtime,
...
#     (MoveApps, MarkdownViewer, RTKInfos, ...), not per-project. Same Apple
#     ID/team, so it works for AppsSwitch too with zero extra setup. Only
#     needed again if this credential is ever revoked:
...
#
# No Sparkle: AppsSwitch has no auto-update feed (v1, distribution manuelle),
# unlike MarkdownViewer — no nested Sparkle.framework to sign here.
#
# Outputs release/AppsSwitch-<version>.dmg, fully notarized.
...
xcodebuild -project AppsSwitch.xcodeproj \
  -scheme AppsSwitch \
...
APP="$ROOT/build/Build/Products/Release/AppsSwitch.app"
if [ ! -d "$APP" ]; then
...
STAGING="$STAGING_DIR/AppsSwitch.app"
...
echo "→ Codesigning nested AppsSwitchCore.framework / AppsSwitchUI.framework"
for FW in "$STAGING/Contents/Frameworks/AppsSwitchCore.framework" "$STAGING/Contents/Frameworks/AppsSwitchUI.framework"; do
...
mkdir -p "$RELEASE_DIR"
DMG="$RELEASE_DIR/AppsSwitch-$VERSION.dmg"
rm -f "$DMG"

echo "→ Creating $DMG"
hdiutil create -volname "AppsSwitch $VERSION" -srcfolder "$STAGING" \
...
echo ""
echo "v1 : distribution manuelle (pas de feed Sparkle/auto-update)."
